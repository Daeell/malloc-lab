...
#define NEXT_FREE(p) (*(char **)(p + WSIZE))
...
find_fit(size_t asize)
{
    ...
    for (bp = freeListPtr; GET_ALLOC(HDRP(bp)) == 0; bp = NEXT_FREE(bp))
    {
...
static void free_list_add(void *ptr)
{
    NEXT_FREE(ptr) = freeListPtr;
    PREV_FREE(freeListPtr) = ptr;
    PREV_FREE(ptr) = NULL;
    freeListPtr = ptr;
}
...
static void free_list_delete(void *ptr)
{
    if (PREV_FREE(ptr) == NULL) // if ptr points to root of free_list
        freeListPtr = NEXT_FREE(ptr);
