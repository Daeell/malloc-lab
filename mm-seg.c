...
static void insert_node(void *bp, size_t size)
{
    ...
    // search_ptr이 할당되어있으면 null이 아니겠지?
    ...
    if (search_ptr != NULL)
    {
        if (insert_ptr != NULL)
        {                                      // 앞뒤가 모두 할당된 블록인 경우
            SET_PTR(NEXT_PTR(bp), search_ptr); // 가용 리스트의 ptr의 next를 뒷 블록 주소로 변경
            SET_PTR(PREV_PTR(search_ptr), bp); // 뒷 블록의 앞 주소를 ptr로 변경
            SET_PTR(PREV_PTR(bp), insert_ptr); // ptr의 앞 주소를 앞 블록 주소로 변경
            SET_PTR(NEXT_PTR(insert_ptr), bp); // 앞 주소의 뒷 주소를 ptr로 변경
        }
        else
        {                                      // 앞이 비었고 뒤가 할당된 경우
            SET_PTR(NEXT_PTR(bp), search_ptr); // ptr의 다음 주소를 뒷 블록으로 변경
            SET_PTR(PREV_PTR(search_ptr), bp); // 뒷 블록의 앞 주소를 ptr로 변경
            SET_PTR(PREV_PTR(bp), NULL);       // ptr의 앞 주소를 null로 변경
            segregated_free_lists[idx] = bp;   // 가용 리스트의 인덱스에 ptr 업데이트. 앞 포인터가 갱신되는 상황이니까?
        }
    }
    else
    {
        if (insert_ptr != NULL)
        {                                      // 앞이 할당되었고 뒤가 비어있는 경우
            SET_PTR(NEXT_PTR(bp), NULL);       // ptr의 뒷 주소를 null로 변경
            SET_PTR(PREV_PTR(bp), insert_ptr); // ptr의 앞 주소를 앞 블록으로 변경
            SET_PTR(NEXT_PTR(insert_ptr), bp); // 앞 블록의 뒷 주소를 ptr로 변경
        }
        else
        {                                    // 둘다 비어있는 경우
            SET_PTR(NEXT_PTR(bp), NULL);     // ptr의 앞 주소 null로 변경
            SET_PTR(PREV_PTR(bp), NULL);     // ptr의 뒷 주소 null로 변경
            segregated_free_lists[idx] = bp; // 가용 리스트의 인덱스에 ptr 업데이트. 앞 포인터가 갱신되는 상황이니까?
        }
    }
